...
export default function useFetchCollectionNfts(_collectionAddress) {
  return useQuery(['collection_nfts', _collectionAddress], () =>
    getCollectionNfts(_collectionAddress),
    { enabled: _collectionAddress &&_collectionAddress !== ZERO_ADDRESS }
  );
}

const getCollectionNfts = async (_collectionAddress) => {
  try {
    ...
    const data = await marketContract.fetctNFTsByCollection(_collectionAddress);
